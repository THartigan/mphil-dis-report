...
The front page now supports an automatic `Word Count:` line if LaTeX is run with shell escape, with the count policy matching the standard rules:

Include:
* summary/abstract
* tables
* footnotes
* appendices (all, except the dedicated auto-generation usage appendix)
...
* bibliography
* acknowledgements
* front matter title page metadata


1. Keep `\wordcount{...}` commented out in `thesis-info.tex` (manual override).
