...
Future work should address these limitations through calibrated photon noise and task-level evaluation of clinical features. Equal-epoch, multi-seed training would also separate dataset size from exposure and quantify uncertainty. Comparing each stochastic PaDIS score estimate with an average over several randomly shifted patch layouts would also enable investigation as to whether its variance affects reconstruction. External-dataset evaluation and expert assessment would then finally be needed to establish clinical validity.
